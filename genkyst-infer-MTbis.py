...
def get_args():
    ...
    parser.add_argument('-e', '--epochs', type=int, default=5, dest='epochs') # 200
...
if __name__ == "__main__":
    ...
    # Modification chemin 
    args.output = "/home/hemery/code/results/genkyst/train/"  
    ...
    args.model = os.path.abspath(args.output + 'epoch.pth')

    # Vérifier si le fichier existe avant de charger
    if not os.path.exists(args.model):
        logging.warning(f"Le fichier {args.model} n'existe pas")

        # Recherche automatique d'un modèle dans le bon répertoire
        model_candidates = sorted(glob.glob(os.path.join(args.output, "../*", "epoch.pth")), key=os.path.getmtime, reverse=True)

        if model_candidates:
            args.model = model_candidates[0]
            logging.info(f"Utilisation du modèle trouvé : {args.model}")
        else:
            raise FileNotFoundError(f"Aucun fichier epoch.pth trouvé dans {args.output}")
